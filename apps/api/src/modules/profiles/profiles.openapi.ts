...
const statement = z.object({ customerProfileId: z.string().uuid(), companyNameEn: z.string().nullable(), periodStart: z.string(), periodEnd: z.string(), currency: z.string(), openingBalance: z.string(), invoicedAmount: z.string(), paymentsAmount: z.string(), creditsAmount: z.string(), closingBalance: z.string(), ageing: z.object({ current: z.string(), d1to30: z.string(), d31to60: z.string(), d61to90: z.string(), over90: z.string() }), invoices: z.array(z.object({ id: z.string().uuid(), invoiceNumber: z.string(), invoiceType: z.string(), issueDate: z.string(), dueDate: z.string(), totalAmount: z.string(), outstandingAmount: z.string(), status: z.string() })), movements: z.array(z.object({ occurredAt: z.string(), description: z.string(), debit: z.string(), credit: z.string(), reference: z.string().nullable() })) }).openapi('CustomerStatement');
registry.registerPath({ method: 'get', path: '/customers/{id}/statement', tags: ['customers'], summary: 'Accounts-receivable statement for a period: opening balance, invoices, payments, credits, closing balance, ageing (same ledger as /credit)', security: bearer, request: { params: idParams, query: statementQuery }, responses: { 200: ok(statement, 'CustomerStatementEnvelope'), 404: err('NOT_FOUND') } });
