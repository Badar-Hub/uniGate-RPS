...
const booking = z
  .object({
    ...
    billingMode: z.enum(['PREPAID', 'INVOICED']), invoiceId: z.string().uuid().nullable(), invoiceNumber: z.string().nullable(), creditTermsDaysSnapshot: z.number().int().nullable(), fulfilmentSequence: z.number().int(), status: z.string(), paymentStatus: z.string(), paymentDueBy: z.string().datetime().nullable(), nonCircumventionUntil: z.string().datetime().nullable(), confirmedAt: z.string().datetime().nullable(),
