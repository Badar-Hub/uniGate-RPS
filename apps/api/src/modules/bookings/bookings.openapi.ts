...
const disputeResult = z.object({ booking: z.object({}).openapi({ $ref: '#/components/schemas/Booking' } as never), complaint: z.object({}).openapi({ $ref: '#/components/schemas/Complaint' } as never), refund: z.object({ id: z.string().uuid(), refundNumber: z.string(), status: z.string(), amount: z.string(), currency: z.string() }).nullable() }).openapi('BookingDisputeResult');
registry.registerPath({ method: 'post', path: '/bookings/{id}/no-show', tags: ['bookings'], summary: 'Ops records a customer or owner no-show: NO_SHOW cancellation row with the policy charge for that party; an owner no-show refunds the customer in full and becomes a PENALTY line on the next settlement (OQ-05); Idempotency-Key required', security: bearer, request: { params: idParams, body: json(noShowBody) }, responses: { 200: ok(cancelResult, 'CancelBookingResultEnvelope'), 403: err('PERM_DENIED'), 409: err('BOOKING_ALREADY_CANCELLED'), 422: err('BOOKING_INVALID_TRANSITION') } });
registry.registerPath({ method: 'post', path: '/bookings/{id}/dispute', tags: ['bookings'], summary: 'IN_PROGRESS / COMPLETED → DISPUTED with a linked complaint (party → global)', security: bearer, request: { params: idParams, body: json(disputeBookingBody) }, responses: { 200: ok(disputeResult, 'BookingDisputeResultEnvelope'), 422: err('BOOKING_INVALID_TRANSITION') } });
registry.registerPath({ method: 'post', path: '/bookings/{id}/resolve-dispute', tags: ['bookings'], summary: 'DISPUTED → COMPLETED (service stands) or a DISPUTE_RESOLVED refund request whose completion moves the booking to REFUNDED; the linked complaints are resolved', security: bearer, request: { params: idParams, body: json(resolveDisputeBody) }, responses: { 200: ok(disputeResult, 'BookingDisputeResultEnvelope'), 422: err('BOOKING_INVALID_TRANSITION / PAYMENT_NOT_REFUNDABLE') } });
