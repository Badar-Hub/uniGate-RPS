...
export const SETTINGS: readonly SettingDefinition[] = [
  ...
  def({ key: 'booking.rating_window_days', section: 'booking', valueType: 'INTEGER', scope: 'PUBLIC', schema: int(1, 90), seed: 14, descriptionEn: 'Days after completion during which a booking can be rated (A-10)', descriptionAr: 'عدد الأيام بعد الإكمال التي يمكن خلالها تقييم الحجز' }),
  def({ key: 'booking.rating_review_below_score', section: 'booking', valueType: 'INTEGER', scope: 'INTERNAL', schema: int(0, 5), seed: 0, descriptionEn: 'Ratings with a comment and a score at or below this land in PENDING_REVIEW before publishing (0 = publish immediately)', descriptionAr: 'التقييمات المصحوبة بتعليق ودرجة تساوي هذه القيمة أو أقل تُعرض للمراجعة قبل النشر (0 = نشر فوري)' }),
  ...
  // Bank transfer (IBFT): shown to a customer who picks BANK_TRANSFER; the method is offered only when the IBAN is set.
  def({ key: 'finance.bank_transfer.bank_name', section: 'finance', valueType: 'STRING', scope: 'INTERNAL', schema: z.string().max(120), seed: '', descriptionEn: 'Bank holding UniGate\'s receiving account for customer transfers (empty = bank transfer not offered)', descriptionAr: 'البنك الذي يستضيف حساب يونيجيت لاستقبال تحويلات العملاء (فارغ = التحويل البنكي غير متاح)' }),
  def({ key: 'finance.bank_transfer.account_name', section: 'finance', valueType: 'STRING', scope: 'INTERNAL', schema: z.string().max(160), seed: '', descriptionEn: 'Account holder name printed with the transfer instructions', descriptionAr: 'اسم صاحب الحساب المعروض مع تعليمات التحويل' }),
  def({ key: 'finance.bank_transfer.iban', section: 'finance', valueType: 'STRING', scope: 'INTERNAL', schema: z.string().regex(/^SA\d{2}[0-9A-Z]{20}$/).or(z.literal('')), seed: '', descriptionEn: 'IBAN customers transfer to (empty = bank transfer not offered even if enabled in finance.payment_methods_enabled)', descriptionAr: 'رقم الآيبان الذي يحوّل إليه العملاء (فارغ = التحويل البنكي غير متاح حتى لو كان مفعّلًا)' }),
  def({ key: 'finance.bank_transfer.instructions_en', section: 'finance', valueType: 'STRING', scope: 'INTERNAL', schema: z.string().max(1000), seed: 'Transfer the exact amount and quote the payment number as the transfer reference. Upload the receipt so finance can verify it.', descriptionEn: 'Instructions shown to the customer (English)', descriptionAr: 'التعليمات المعروضة للعميل (إنجليزي)' }),
  def({ key: 'finance.bank_transfer.instructions_ar', section: 'finance', valueType: 'STRING', scope: 'INTERNAL', schema: z.string().max(1000), seed: 'حوّل المبلغ بالضبط واذكر رقم الدفعة كمرجع للتحويل، ثم ارفع الإيصال ليتحقق منه قسم المالية.', descriptionEn: 'Instructions shown to the customer (Arabic)', descriptionAr: 'التعليمات المعروضة للعميل (عربي)' }),
  def({ key: 'finance.bank_transfer.receipt_window_hours', section: 'finance', valueType: 'INTEGER', scope: 'INTERNAL', schema: int(1, 720), seed: 48, descriptionEn: 'Hours a bank-transfer payment stays open for the receipt before it expires', descriptionAr: 'عدد الساعات التي تبقى فيها دفعة التحويل البنكي مفتوحة لرفع الإيصال قبل انتهائها' }),
  ...
  def({ key: 'onboarding.owner_self_registration_enabled', section: 'onboarding', valueType: 'BOOLEAN', scope: 'PUBLIC', schema: bool, seed: false, descriptionEn: 'Allow the public registration form to create vehicle-owner (vendor) accounts; off = only admins add vendors', descriptionAr: 'السماح لنموذج التسجيل العام بإنشاء حسابات مالكي المركبات (الموردين)؛ إيقافه = المشرفون فقط يضيفون الموردين' }),
  ...
  def({ key: 'notifications.enabled_channels', section: 'notifications', valueType: 'STRING_ARRAY', scope: 'INTERNAL', schema: z.array(z.enum(['IN_APP', 'EMAIL', 'SMS', 'PUSH'])).min(1), seed: ['IN_APP', 'EMAIL', 'SMS', 'PUSH'], descriptionEn: 'Channels the platform delivers on; a channel not listed is SUPPRESSED (the provider is deployment config)', descriptionAr: 'القنوات التي تُرسل عليها المنصة؛ القناة غير المدرجة تُوقف' }),
  def({ key: 'notifications.locked_categories', section: 'notifications', valueType: 'STRING_ARRAY', scope: 'INTERNAL', schema: z.array(z.string().regex(/^[A-Z_]{2,48}$/)), seed: ['SECURITY', 'PAYMENT', 'TRIP'], descriptionEn: 'Transactional categories users cannot opt out of (FR-NOTIFICATIONS-06)', descriptionAr: 'الفئات التشغيلية التي لا يمكن للمستخدم إيقافها' }),
  def({ key: 'notifications.urgent_categories', section: 'notifications', valueType: 'STRING_ARRAY', scope: 'INTERNAL', schema: z.array(z.string().regex(/^[A-Z_]{2,48}$/)), seed: ['SECURITY', 'PAYMENT', 'TRIP'], descriptionEn: 'Categories delivered immediately even inside quiet hours', descriptionAr: 'الفئات التي تُرسل فوراً حتى خلال ساعات الهدوء' }),
  def({ key: 'notifications.retention_days', section: 'notifications', valueType: 'INTEGER', scope: 'INTERNAL', schema: int(7, 730), seed: 180, descriptionEn: 'Days an in-app notification stays in the inbox before the purge job removes it', descriptionAr: 'عدد الأيام التي يبقى فيها الإشعار في صندوق الوارد قبل حذفه' }),
  def({ key: 'notifications.maintenance_reminder_days_before', section: 'notifications', valueType: 'INTEGER', scope: 'INTERNAL', schema: int(0, 90), seed: 7, descriptionEn: 'Days before a maintenance schedule falls due at which the owner is reminded', descriptionAr: 'عدد الأيام قبل موعد الصيانة لتذكير المالك' }),
  def({ key: 'notifications.maintenance_reminder_km_before', section: 'notifications', valueType: 'INTEGER', scope: 'INTERNAL', schema: int(0, 10_000), seed: 500, descriptionEn: 'Kilometres before a maintenance schedule falls due at which the owner is reminded', descriptionAr: 'عدد الكيلومترات قبل موعد الصيانة لتذكير المالك' }),
  ...
  // ── platform ───────────────────────────────────────────────────────────────
  def({ key: 'platform.complaint_categories', section: 'platform', valueType: 'STRING_ARRAY', scope: 'PUBLIC', schema: z.array(z.string().regex(/^[A-Z_]{2,48}$/)).min(1), seed: ['SERVICE_QUALITY', 'SAFETY', 'DRIVER_BEHAVIOUR', 'VEHICLE_CONDITION', 'DELAY', 'DAMAGE_OR_LOSS', 'BILLING', 'PLATFORM', 'OTHER'], descriptionEn: 'Complaint categories offered when raising a complaint', descriptionAr: 'فئات الشكاوى المتاحة عند رفع شكوى' }),
  def({ key: 'platform.complaint_sla_hours', section: 'platform', valueType: 'JSON', scope: 'INTERNAL', schema: z.object({ LOW: int(1, 720), MEDIUM: int(1, 720), HIGH: int(1, 720), CRITICAL: int(1, 720) }), seed: { LOW: 120, MEDIUM: 72, HIGH: 24, CRITICAL: 4 }, descriptionEn: 'First-response SLA per complaint severity (hours) — drives the overdue flag in the queue', descriptionAr: 'مهلة الاستجابة الأولى حسب خطورة الشكوى (ساعات)' }),
