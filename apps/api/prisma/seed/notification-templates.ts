...
export const NOTIFICATION_TEMPLATES: TemplateSeed[] = [
  ...
  { code: 'BANK_TRANSFER_RECEIPT_SUBMITTED', category: 'PAYMENT', channels: ['IN_APP', 'EMAIL'], variables: ['paymentNumber', 'amount', 'currency'],
    en: { title: 'Bank transfer receipt for {{paymentNumber}}', body: 'A customer submitted a receipt for {{amount}} {{currency}} ({{paymentNumber}}). Verify it against the bank statement in Admin → Payments → Bank transfers.' },
    ar: { title: 'إيصال تحويل بنكي للدفعة {{paymentNumber}}', body: 'أرسل عميل إيصالًا بقيمة {{amount}} {{currency}} ({{paymentNumber}}). تحقق منه مقابل كشف الحساب البنكي في الإدارة ← المدفوعات ← التحويلات البنكية.' } },
