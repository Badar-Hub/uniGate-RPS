...
`calculation_type = NONE` means *charge no commission* — it is a first-class value, not a zero-rate percentage, so "we chose not to charge" is distinguishable from "we charge 0%" in every report. A `CHECK` enforces that `PERCENTAGE` carries `percentage_rate` as a fraction in `[0, 1]` (the API takes and shows it as a percent), `FIXED` carries `fixed_amount ≥ 0`, and `NONE` carries neither.
