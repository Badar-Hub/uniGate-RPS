...
| POST | `/payments/{id}/receipt` **⧗** | `payments.create` | own(customer) → global | **Bank transfer (IBFT).** The payer attaches the receipt: `{ documentId, transferReference?, transferredAt? }` where `documentId` is an `UPLOADED` document on target `PAYMENT/{id}` (type `PAYMENT_RECEIPT`). Sets `bankTransfer.awaitingVerification`, stops the expiry clock and notifies finance (`payment.receipt_submitted`). May be repeated while `PENDING` (replaces the receipt). Never changes the payment status. |
| POST | `/admin/payments/{id}/verify-transfer` **⧗** | `payments.manage` | global | Finance matched the credit on the bank statement: `{ notes? }`. Runs the same capture path as a webhook (`PAID`, booking `CONFIRMED`, ledger against `CASH_BANK`). `422 PAYMENT_INVALID_TRANSITION` unless the payment is a `PENDING` bank transfer. |
| POST | `/admin/payments/{id}/reject-transfer` **⧗** | `payments.manage` | global | No matching credit: `{ reason }` (≥ 5 chars, shown to the customer). Payment → `FAILED` with `failureCode = TRANSFER_REJECTED`; the booking returns to `PENDING_PAYMENT` and the customer may pay again. |
| GET | `/payments/config` | `payments.read` | — | Client-safe payment configuration: enabled `methodTypes`, provider code, publishable key, currency, 3-DS behaviour, and `bankTransfer` (receiving bank, account name, IBAN, instructions, receipt window) when `BANK_TRANSFER` is offered — i.e. enabled in `finance.payment_methods_enabled` **and** `finance.bank_transfer.iban` is set. `PUT /payments/config` requires `payments.config.manage`. **Never returns a secret** (`system_settings.scope = SECRET` is filtered out unconditionally). |
...
| POST | `/maintenance/records/{id}/complete` **⧗** | `maintenance.update` | own → global | → `COMPLETED`, sets `actual_end_at`, releases the calendar entry, updates the vehicle's `odometer_km` (never backwards → 422) and `operational_status` (→ `IDLE`), rolls the linked `maintenance_schedules` row forward and — `recordExpense` (default true) — books the cost as a `MAINTENANCE` expense of the owner. `POST …/start` (PLANNED → IN_PROGRESS, vehicle `UNDER_MAINTENANCE`) and `POST …/cancel` (reason, releases the hold) share the route family. COMPLETED / CANCELLED are final (`422 MAINTENANCE_INVALID_TRANSITION`). |
| DELETE | `/maintenance/records/{id}` | `maintenance.delete` + `maintenance.read_any` | global | Only for `PLANNED` records created in error (staff — owners cancel instead). Releases the calendar entry. `204`. |
...
| POST | `/notifications/devices` | `notifications.read` | self | Register a push device token (`DELETE /notifications/devices/{token}` deactivates). Tokens are stored before a push provider is configured; with `PUSH_PROVIDER=none` PUSH rows are `SUPPRESSED`; `PUSH_PROVIDER=expo` relays `ExponentPushToken[…]` tokens through the Expo Push Service (tickets with `DeviceNotRegistered` deactivate the token). |
...
- **Late invitations.** Eligibility changes after publish are not lost: the event worker re-runs the matcher for still-open requests (`PUBLISHED` / `PARTIALLY_AWARDED`, bidding not closed) on `vehicle.approved`, `owner.approved` and `owner.service_areas_replaced`, writes the missing `trip_request_invitations` (`matchReason.reasons` gains `LATE:<event>`) and emits `trip_request.invitations_added` so only the newly invited owners receive `NEW_TRIP_OPPORTUNITY`.
...
| `methodType` | `MADA`, `VISA`, `MASTERCARD`, `STC_PAY`, `APPLE_PAY`, `BANK_TRANSFER`, `CASH`. Must be enabled in `GET /payments/config`. `BANK_TRANSFER` is offered only when `finance.bank_transfer.iban` is set and returns `action.type: "NONE"` — the client shows the receiving account from `GET /payments/config` (`bankTransfer`), the customer uploads the receipt (`POST /payments/{id}/receipt`) and the payment stays `PENDING` until finance verifies it (`POST /admin/payments/{id}/verify-transfer`) or rejects it. |
| `savedMethodId` | Optional `payment_method_tokens` id owned by the actor. |
| `returnUrl` | Must match an allow-listed origin (`APP_URL`, `CORS_ORIGINS`) **or the mobile app's URL scheme** (`MOBILE_DEEP_LINK_SCHEME`, default `unigate://…`, opened from an in-app browser session — ADR-011). An open redirect here is a phishing primitive. |
