...
| FR-BIDDING-14 | Check the corporate credit limit inside the award transaction, with the customer row locked, before an `INVOICED` booking is created | SYS | `CLIENT 2026-09-14` (A-46) | MUST | 7 | Outstanding receivables plus this booking's total must stay within `credit_limit_amount`; outstanding is read from the ledger, never from a cached column. 422 `RULE_CREDIT_LIMIT_EXCEEDED` or `RULE_CREDIT_NOT_APPROVED`. Breach and overdue policy is **OQ-21** |
...
| FR-BOOKINGS-09 | Cancel a booking with a reason code, recording hours before pickup and the fee rule applied as a snapshot | C, O, A | RFP §3 / BRIEF-§13 | MUST | 8 | Fee tiers are **OQ-05**; the rule is snapshotted, never hard-coded |
...
| FR-BOOKINGS-12 | Snapshot `billing_mode` on the booking at award and key the legal transition map on it | SYS | `CLIENT 2026-09-14` (A-46) | MUST | 8 | Same pattern as the trip transition map keyed on `transport_type`. A later change to the customer's billing arrangement cannot retroactively alter a booking's lifecycle |
...
| ID | Requirement | Actor(s) | Source | Priority | Phase | Notes |
|---|---|---|---|---|---|---|
| FR-FINANCE-01 | Configure commission rules as percentage or fixed amount, scoped globally or by vehicle category, owner, or owner and category | A | RFP §3 / BRIEF-§18 | MUST | 11 | Rate and basis are **OQ-01**; the engine is built, the number is seeded |
...
Background:
  ...
  And an active global commission rule of 10 percent on a net-of-VAT basis   # A-02, pending OQ-01
...
| R-05 | **Payment gateway selection has external lead time.** Merchant onboarding and sandbox access typically take weeks and sit on the Phase 9 critical path | High | High | Start gateway selection and merchant onboarding now, in parallel with Phases 2–8 — OQ-03. `MockGateway` allows all of Phases 8–11 to be built and tested without it; only certification is blocked | Finance lead |
...
| R-07 | **Cancellation and refund policy is undefined but customer-visible from the first booking.** The interim assumption of free cancellation is commercially generous and hard to withdraw once customers have experienced it | High | Medium | Snapshot the applied rule per cancellation so real tiers apply cleanly from the day they are set — OQ-05. Flag to the client that the interim policy is live behaviour, not a placeholder, and sets expectations | Commercial lead |
