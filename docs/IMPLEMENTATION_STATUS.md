...
| 16 | Deployment | **IN PROGRESS** 2026-09-20 | [deployment.md](deployment.md): API/web images, single-host compose behind a TLS proxy (private CA for IP-only hosts), migrate-before-start, deploy script; Android signed APK/AAB build script + EAS profiles; store-submission runbook. Deployed on the UAT VM (`staging` tier). Hosting region still **BLOCKED** on OQ-12; production tier waits on the payment gateway, SMS vendor (B-9) and SMTP relay |
