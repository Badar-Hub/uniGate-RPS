...
Eighteen modules. Each owns its tables, exposes a service, and publishes domain events.
...
| `demand` | trip_requests, passenger/goods details, invitations | Request lifecycle and opportunity matching | 6 |
...
| `trips` | trips, status history, proofs | Operational execution, POD | 10 |
...
| finance | `commissions.read` `commissions.manage` `settlements.read` `settlements.create` `settlements.approve` `settlements.pay` `invoices.read` `invoices.issue` `ledger.read` |
...
| AR-2 | Payment gateway unselected (**OQ-03**) | Phase 9 cannot reach production | Interface-first; `MockGateway` exercises the whole flow; adapter is ~1–2 weeks once chosen |
| AR-3 | GPS vendor unselected (**OQ-11**) | Hardware tracking path unbuilt | Driver-app GPS is fully sufficient for launch; adapter slots into the same normalised pipeline |
...
| AR-11 | **The e-invoicing integration deadline may already have passed** (**OQ-04**) | Wave 24 (revenue > SAR 375,000 in 2022–2024) required integration by **30 June 2026**. If UniGate fell in that wave rather than Wave 25 (deadline 1 Feb 2027), it is already late — a live regulatory exposure independent of this project. **Recalibrated 2026-09-14 from acute to material:** the published violation ladder is warning-first — first occurrence is a **notice with no fine** and **up to three months to correct**, then 1,000 / 5,000 / 10,000 / 40,000 SAR, resetting after 12 months — and there is no separately-named penalty for missing a wave deadline. Enforcement is real and at scale (>61,000 field visits in Q2 2026, e-invoicing consistently a named violation category) but remediation-friendly. Note the fines-exemption initiative running to 31 Dec 2026 **appears to have dropped** the e-invoicing coverage the previous round carried | **The wave is decided by data UniGate already holds**: test 2022, 2023 and 2024 VAT-taxable revenue **separately** against SAR 375,000 — a single qualifying year captures you. ZATCA notifies by **email and SMS** to ERAD-registered contacts, not by letter, so check those contact details first; there is no wave-lookup tool in the Fatoora portal. Then **onboard early regardless of the answer** — it is expressly permitted and officially recommended, and the portal gates only on active VAT registration, not wave membership. Do **not** bulk back-fill gap-period invoices without a written ZATCA position: sequence gaps are flagged for investigation and the PIH/ICV chain conflict is unaddressed in guidance |
| AR-12 | **Per-owner VAT treatment implies per-owner ZATCA certificate onboarding** (**OQ-24**) | A CSID is bound to one VAT number and is cross-checked against the seller VAT in the invoice XML, so an `OWNER_IS_SUPPLIER` invoice must be signed with the owner's certificate — one EGS unit, keypair, CSID and independent ICV/PIH chain per registered owner. The Fatoora OTP that starts it **has no API** and must be generated by the owner personally within a one-hour window, at onboarding and at every renewal | [ADR-008](decisions/ADR-008-vat-operating-model.md) proposes uniform principal treatment to collapse this to one certificate. `vat_treatment` is retained so the per-owner path stays available. **Per-owner CSID onboarding is deliberately not built pending the advisor's answer** |
...
| AR-7 | Mobile scope divergence between RFP §5 and the brief (**OQ-14**) | Contractual exposure | API is client-agnostic and documented via OpenAPI; requires written confirmation |
