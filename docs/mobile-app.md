# The mobile app — Expo (React Native) for customers, vehicle owners and drivers

**Status:** M3 delivered 2026-09-17 — the driver surface in the same app (§15): role-aware driver tabs, my trips with upcoming / active / done, the trip screen with the status transitions exactly as the PWA does them (odometer, proof of delivery, Bayan), proofs with photo / signature attachments through the documents flow, status history, the **background live-location agent** (`expo-location` + `expo-task-manager`, Android foreground service, iOS `UIBackgroundModes: location`) with the PWA's throttle / offline queue / batch flush, the driver account (availability, licence warning, assignments, location permission) and driver-aware deep links. M2 (2026-09-17) was the vendor (vehicle-owner) journey on the phone: opportunities with detail and dismiss / undo, bid submit / revise / withdraw with server-computed totals, owner dispatch on the booking (assign driver, ready) with the owner's financial projection, the fleet with vehicle registration, the document checklist with camera / gallery / PDF upload to presigned URLs, calendar blocks and driver assignments, drivers with registration, verticals and availability, the owner profile with service areas and documents, settlements, expenses with receipts and maintenance records, and an owner Home summary (§14). M1 (2026-09-17) was the customer journey end to end: new request (passenger and goods), request detail, bid comparison with accept / group award / reject, booking detail with cancel-with-quote, pay-now through the gateway hand-off and return polling, live tracking over Socket.IO with a map, ratings and complaints, the corporate credit / invoices / statement screens, saved locations, the inbox with an unread badge and deep links, and push-device registration. M0 (2026-09-16) was the scaffold: sign-in (password and phone OTP), session bootstrap with transparent refresh, role-aware tabs, Arabic/English with RTL. Decision record: [ADR-011](decisions/ADR-011-mobile-expo-over-pwa.md). The driver [PWA](driver-app.md) stays deployed until the lead retires it; §15.4 lists the parity reached.
...
| Location   | `expo-location` + `expo-task-manager` + `@react-native-async-storage/async-storage`                                                                                                 | The driver's live-location agent (§15.2): background task + Android foreground service, high accuracy at a 5 s cadence, the offline queue persisted in AsyncStorage                    |
...
│     │  ├─ trips.tsx         GET /trips?status= · GET /trips/active   the driver's trips: active / upcoming / done (§15)
│     │  ├─ active.tsx        the driver's home: the active trip with the agent's state, else the next trips (§15)
│     │  └─ account.tsx       profile, Saved locations, Complaints, Company / Invoices / Statement (corporate), the vendor section (§14), the driver section (§15), language, sign out
...
│     ├─ trips/[id].tsx       GET /trips/{id}, …/status-history, …/proofs; POST …/status ⧗, …/proofs; PATCH /trips/{id} (Bayan); the location agent  (§15)
│     └─ account/driver.tsx   GET /drivers/{id}, …/assignments; POST …/availability; location permission; sign out
├─ src/
...
│  ├─ lib/driver/transitions.ts  the vertical transition tables, button order, odometer / proof / Bayan requirements, the status body — pure, unit-tested (§15.1)
│  ├─ lib/driver/tracker-core.ts the agent's throttle / offline queue / batch flush with injected transport and storage — pure, unit-tested with fake timers (§15.2)
│  ├─ lib/driver/tracker.ts   the native agent: expo-location background task (`unigate-driver-location`) or foreground watch, permissions, AsyncStorage queue, one-shot fix for transitions
...
│  ├─ hooks/use-tracker.ts    the agent's state for a screen
...
│  ├─ components/driver/      trip-card · driver-home · location-card (the agent) · trip-actions (status buttons + form) · trip-proofs (list, record, attach)
...
Test accounts: the same seed users as the portal (`pnpm db:seed`; see `uat-checklist.md`). A customer account gets the customer tabs; an owner gets the vendor tabs; an account with both profiles gets both; a driver (phone OTP sign-in) gets Trips / Active / Notifications / Account, and a driver who also has another profile keeps that set plus a Trips tab (`tabsFor`, §15).
...
| `tripId` | `/track/{id}` — or `/trips/{id}` (the driver's trip screen) when the signed-in user has a driver profile (`DeepLinkContext`, §15.3) | `unigate://track/{id}` · `unigate://trips/{id}` |
...
| **M3** ✓  | The driver surface (§15): driver tabs, my trips, the trip screen with the PWA's status transitions, proofs with attachments, status history, the **background live-location agent** (`expo-location` + `expo-task-manager`, Android foreground service, iOS background mode) with the PWA's throttle / queue / batch flush, the driver account, driver-aware deep links. Still open: the driver PWA's retirement (the lead's call — §15.4 lists parity); a screen-awake hint (`expo-keep-awake`) for the foreground-only fallback; verifying the background service on a physical device with a development build (Expo Go cannot register background tasks); everything listed under M2 as still open |
...
## 15. The driver journey (M3)

The driver surface lives in the same app: a user with `me.profiles.driver` gets the tab set **Trips / Active / Notifications / Account** (a driver who also has a customer or owner profile keeps their set plus a Trips tab — `tabsFor` in `src/lib/tabs.ts`, unit-tested). The Active tab is the driver's home: the active trip's card with the agent's state (`GET /trips/active`), else the next trips soonest first (`GET /trips?status=BOOKED,DRIVER_ASSIGNED`); Home shows the same block at the top for a driver with other profiles. The behavioural reference is the PWA (`apps/web/src/components/driver/{driver-trips,driver-trip}.tsx`, `apps/web/src/lib/driver/tracker.ts`): the same endpoints, the same bodies, the same disabled-button rules.

| Screen | Calls | Notes |
| --- | --- | --- |
| Trips tab | `GET /trips/active` (Active), `GET /trips?status=BOOKED,DRIVER_ASSIGNED` (Upcoming), `GET /trips?status=COMPLETED,CANCELLED` (Done), all with `page` / `pageSize` | The driver scope of `/trips` returns only the signed-in driver's trips (api.md §8.15). Offset list with pull-to-refresh and load-more; tap → the trip. |
| Trip | `GET /trips/{id}`, `GET /trips/{id}/status-history`, `GET /trips/{id}/proofs` | Booking summary (pickup / drop-off with *Open in maps*, the scheduled window, vehicle, vertical, Bayan reference, notes, odometer, actual times and distance), the location agent's card, the status buttons, the proofs, the history with the position captured at each step. **The DTO carries no customer contact** (only `customerProfileId`), so none is shown — the same as the PWA. |
| Status transition | `POST /trips/{id}/status` ⧗ `{ status, occurredAt, latitude?, longitude?, accuracyM?, note?, odometerKm?, proofId? }`; before it when needed: `POST /trips/{id}/proofs { proofType: DELIVERY_CONFIRMATION, recipientName, recipientIdLast4?, notes?, latitude?, longitude? }` (goods → DELIVERED) and `PATCH /trips/{id} { regulatoryReference, regulatoryReferenceType: BAYAN }` (goods → DRIVER_EN_ROUTE without a reference) | The buttons are exactly `allowedNextStatuses` minus `CANCELLED` (ops-only), ordered along the vertical's happy path with *Report a problem* (EXCEPTION) last (`driverActions`). The odometer is asked on the plugins' `odometerRequiredOn` (passenger: TRIP_STARTED, COMPLETED; goods: LOADED, DELIVERED) and refused below the floor; the proof form before DELIVERED; a note on every step. **Every transition carries the device position** — the agent's last fix when under 30 s old, else one `getCurrentPositionAsync` read — which the API writes to `trip_status_history` (security.md T-30, dispute evidence). `details.allowed` from `TRIP_INVALID_TRANSITION` is shown in the banner. |
| Proofs | `GET /trips/{id}/proofs`, `POST /trips/{id}/proofs { proofType, recipientName?, recipientIdLast4?, notes?, latitude?, longitude? }`, then the §14.2 upload flow with `target { kind: TRIP_PROOF, id: <proofId> }` and the `POD_PHOTO` / `POD_SIGNATURE` types (`GET /reference/document-types?appliesTo=TRIP_PROOF`) | A proof references documents through `documents.trip_proof_id`, and the upload-url target check requires the proof row to exist on the driver's own trip — so the proof is always recorded first, then *Attach photo* / *Attach signature* upload against it. The proof body's `documentIds` / `signatureDocumentId` are therefore not used by the app (there is no document to reference before the proof exists). |
| Driver account | `GET /drivers/{id}` (self scope), `POST /drivers/{id}/availability { availabilityStatus }` (AVAILABLE ↔ OFF_DUTY; hidden while ON_TRIP), `GET /drivers/{id}/assignments` | The licence warning (expired, or expiring within 30 days), the verticals, the assignment history (vehicle id prefix and dates — the assignment DTO carries no plate), the location permission state with *Open settings*, and sign-out (push deregistered and the agent stopped first). |

### 15.1 The transition table

`src/lib/driver/transitions.ts` mirrors the API's vertical plugins (`apps/api/src/modules/{passenger,goods}/plugin.ts`): the transition maps, `startStatus`, `odometerRequiredOn`, `proofRequiredOn` and the tracked states. It never decides legality — the API's `allowedNextStatuses` and `POST /trips/{id}/status` do — it orders the buttons, says which steps need extra input and builds the status body byte-for-byte as the PWA. The unit test walks each happy path through the table, checks the requirement lists against the plugins, and pins that the driver can never reach `CANCELLED` through a button.

### 15.2 The live-location agent

`src/lib/driver/tracker-core.ts` is the PWA agent's logic with the GPS source and the storage injected (unit-tested with fake timers): one sample every **5 s** at most → `POST /tracking/ping { tripId, latitude, longitude, accuracyM, speedKmh?, headingDeg?, recordedAt, source: DRIVER_APP }` (7 dp coordinates, integer metres / degrees, km/h to 1 dp — `trackingPingBody`); a sample the API could not be reached for (network, 5xx, 429) goes into the **offline queue** (200 max, oldest dropped, persisted in AsyncStorage) and is flushed in recorded order through `POST /tracking/ping/batch { points }` — per-item results: a refused sample (`TRACKING_STALE_POINT`, implausible) is dropped and counted, a throttled one (`RATE_LIMITED`) goes back for the next retry (every 15 s while queued); a business refusal on a single ping (`TRACKING_SESSION_NOT_ACTIVE`, `TRACKING_STALE_POINT`, 404) is never retried — it is counted and shown as the API's plausibility rejection; `TRACKING_SESSION_NOT_ACTIVE` / `NOT_FOUND` also stop the agent (the trip ended while the app was away). The card on the trip screen shows sent / queued, the last fix with age and accuracy, `lowConfidence`, the rejections and the mode.

`src/lib/driver/tracker.ts` owns the device side:

- **Background** (development / store builds): `Location.startLocationUpdatesAsync('unigate-driver-location', { accuracy: High, timeInterval: 5000, distanceInterval: 5, deferredUpdatesInterval: 5000, activityType: AutomotiveNavigation, pausesUpdatesAutomatically: false, showsBackgroundLocationIndicator: true, foregroundService: { notificationTitle, notificationBody, notificationColor } })`. The TaskManager task is defined at module scope (`app/_layout.tsx` imports the module first thing) so a headless relaunch by the OS still runs it: the active trip id is persisted in AsyncStorage, the core is restored and the API client refreshes its access token from SecureStore. Of a deferred burst the newest fix is sent (the API samples history at 30 s / 50 m anyway).
- **Permissions**: foreground ("while using") first, then background ("all the time" — Android opens its settings page; iOS upgrades to Always). A refused background permission degrades to **foreground only** with `watchPositionAsync` and says so on the card with *Open settings*; a refused foreground permission stops with the same button; location services off is reported. The agent starts by itself when the trip enters a tracked state (from DRIVER_EN_ROUTE, when the API opens the tracking session) and stops at COMPLETED / CANCELLED, on sign-out, or when the driver taps *Stop* (a manual override for that screen visit). Leaving the screen does not stop it.
- **Expo Go** cannot register background tasks: `isExpoGo()` (from `src/lib/push.ts`) selects the foreground watch and the card says "Expo Go: foreground only — background sharing needs a development build".
- **Config** (`app.config.ts`): the `expo-location` plugin with the three iOS usage strings, `isIosBackgroundLocationEnabled`, `isAndroidBackgroundLocationEnabled` and `isAndroidForegroundServiceEnabled` (→ `ACCESS_BACKGROUND_LOCATION`, `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_LOCATION`), plus `ios.infoPlist.UIBackgroundModes: ['location']`. `expo-task-manager` needs no plugin.

Cadence note: the ingest limit is documented as 12/min per trip (security.md T-30, "10 s cadence plus slack") while `tracking.service.ts` currently enforces 30/min; the 5 s cadence (12/min) sits exactly on the documented ceiling, so a 429 is treated as transient (queued, retried after `Retry-After`), never as a rejection.

### 15.3 Deep links

`routeForNotification(data, ctx)` / `routeForUrl(url, ctx)` take a `DeepLinkContext { driver }`: a `tripId` hint — and `unigate://trips/{id}` or `unigate://track/{id}` — opens the driver's trip screen for a user with a driver profile and the tracking view for everyone else (unit-tested). The inbox rows pass the session's audience; the push tap handler evaluates it at tap time so a cold-start tap routes correctly once `/me` has loaded.

### 15.4 Parity with the driver PWA

Reached: sign-in with the same accounts, my trips (active first), the trip screen with exactly the API's buttons, the odometer prompt, the proof-of-delivery form before DELIVERED, the Bayan prompt before departure, coordinates on every transition, the live-location agent with the 5 s throttle, the 200-sample offline queue and the per-item batch flush, sent / queued / last-fix / accuracy display, permission handling, Arabic / English. Beyond the PWA: true background tracking with a foreground service, the proofs list with photo / signature attachments, status history, the driver account (availability, licence warning, assignments), push notifications and driver-aware deep links, the Home / Active summary.

What the PWA still has that the app does not: the service-worker **offline shell page** (the app shows the API error and keeps the queue; lists have no offline cache — an M2 open item), the *Add to Home screen* install hint (not applicable), and the **screen wake lock** — not needed with the background service, but in the foreground-only fallback the screen may lock and pause the watch (`expo-keep-awake` is the small follow-up). The PWA's "offline" banner has no equivalent yet (the card reports queued samples instead). Retiring the PWA is the lead's decision; nothing in `apps/web` was changed.
